...
class DigitClassifier(context: Context) {
    ...
    private fun convertBitmapToByteBuffer(bitmap: Bitmap): ByteBuffer {
        ...
        val floatPixels = mutableListOf<Float>()

        bitmap.getPixels(pixels, 0, bitmap.width, 0, 0, bitmap.width, bitmap.height)

        Log.d(TAG, "Pixels: ${pixels.contentToString()}")
        ...
        for (pixelValue in pixels) {
            ...
            // 255 - pixel; to invert color, white background to black background
            val normalizedPixelValue = (255 - ((r + g + b) / 3.0f)) / 255.0f
            floatPixels.add((255 - (r + g + b) / 3.0f) / 255.0f)
            byteBuffer.putFloat(normalizedPixelValue)
        }

        Log.d(TAG, "convertBitmapToByteBuffer: $floatPixels")
